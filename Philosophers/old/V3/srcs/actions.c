...
/*   Updated: 2024/01/22 16:04:25 by gduranti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_sleep(t_diner *diner)
{
	...
	{
		usleep(diner->var->time_to_die);
		ft_mutprint(diner, P_DEATH);
		ft_exit(&diner, TRUE);
	}
	usleep(diner->var->time_to_sleep);
...
void	*ft_think(void *diner1)
{
	...
	else if (diner->id_fork.__data.__lock == FALSE && diner->next->id_fork.__data.__lock == FALSE)
	{
		ft_takefork(diner);
		pthread_mutex_unlock(&diner->var->look);
		ft_eat(diner);
