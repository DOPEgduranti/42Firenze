...
/*   Updated: 2024/01/24 13:13:41 by gduranti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_dinerclear(t_diner **lst)
{
	...
	// 	counter--;
	// }
	if ((*lst)->var->eat.__data.__lock == TRUE)
		pthread_mutex_unlock(&(*lst)->var->eat);
	if ((*lst)->var->look.__data.__lock == TRUE)
		pthread_mutex_unlock(&(*lst)->var->look);
	if ((*lst)->var->print.__data.__lock == TRUE)
		pthread_mutex_unlock(&(*lst)->var->print);
	if ((*lst)->var->finish.__data.__lock == TRUE)
