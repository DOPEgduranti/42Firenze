...
/*   Updated: 2024/02/05 12:44:20 by gduranti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_philo_init(t_diner *diner, t_data *data, int i)
{
	...
	if(ft_threrr(pthread_create(&diner->undertaker, NULL, ft_checkdeath, diner), data) == TRUE)
		exit(EXIT_FAILURE);
	// pthread_detach(diner->undertaker);
	if(ft_threrr(pthread_create(&diner->philokill, NULL, ft_philodied, diner), data) == TRUE)
		exit(EXIT_FAILURE);
	// pthread_detach(diner->philokill);
}
